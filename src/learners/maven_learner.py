...
from src.modules.mixers.noise_mix import QMixer as NoiseQMixer
import numpy as np
import torch as th
...
class MavenLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # state + joint action probability
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        q_softmax_actions = th.nn.functional.softmax(mac_out_clone[:, :-1], dim=3)        # (bs, max_seq_length-1, n_agents, n_actions)

        if self.args.hard_qs:
            maxs = th.max(mac_out_clone[:, :-1], dim=3, keepdim=True)[1]
            zeros = th.zeros_like(q_softmax_actions)
            zeros.scatter_(dim=3, index=maxs, value=1)
            q_softmax_actions = zeros

        q_softmax_agents = q_softmax_actions.reshape(q_softmax_actions.shape[0], q_softmax_actions.shape[1], -1)        # (bs, max_seq_length-1, n_agents*n_actions)

        states = batch["state"][:, :-1]
        state_and_softactions = th.cat([q_softmax_agents, states], dim=2)       # (bs, max_seq_length-1, n_agents*n_actions + state_shape)

        s_and_softa_reshaped = state_and_softactions.reshape(-1, state_and_softactions.shape[-1])       # shape=(bs*(max_seq_length-1), (n_agents*n_actions+state_shape))

        if self.args.mi_intrinsic:      # Default False
        ...
        discrim_target = batch["noise"][:, 0].long().detach().max(dim=1)[1].unsqueeze(1).repeat(1, target_repeats).reshape(-1)      # (bs, target_repeats)
    ...
    def cuda(self):
        ...
        self.discrim.to(self.args.device)
        if self.mixer is not None:
