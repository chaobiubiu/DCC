...
from src.modules.mixers.qmix_hidden import QMixerHidden
import torch as th
...
class MACCLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # 值得注意的是：这里编码ally and enemy units information的fc和GRU层应该都不训练
        ...
        if args.inseparable:
            ...
            if args.env == 'sc2':
                ...
                self.nf_al = 5
                self.nf_en = 5
            # 这里fc_al的input_shape是(nf_al+self.args.n_agents)，是因为global state中相应部分信息外有额外的agent one-hot-id?
            ...
            # 如果task不可分的话，则将所有enemies的信息作为一个整体进行encoding.
            ...
            if args.env == 'sc2':
                ...
                self.nf_al = 5
                self.nf_en = 5
            # fc_al对于ally units的信息处理与上面相同
            self.fc_al = nn.Linear(self.nf_al + self.args.n_agents, args.rnn_hidden_dim_ally).to(self.args.device)
            self.rnn_al = nn.GRUCell(args.rnn_hidden_dim_ally, args.rnn_hidden_dim_ally).to(self.args.device)
            # fc_env对于enemy units的信息处理与上面不同
        ...
        if self.args.mixer is not None:
            if self.args.mixer == "vdn":
                ...
                for i in range(self.n_agents):
                    self.new_states[:, :, i * (5 * self.nf_al + self.n_agents) + 5 * self.nf_al + i] = 1        # 这里应该是将每个agent local state中的one-hot agent id进行编码
            else:
                if self.args.inseparable:
        ...
        for t in range(batch.max_seq_length):
            ...
            if self.args.mixer is not None:
                if self.args.mixer == "vdn":
                    ...
                    for i in range(self.n_agents):
                        if t > 0:
                            # 这里给我的感觉是每个agent的feature dim是 5*nf_al+n_agents，而且第一个nf_al length相应的feature从当前时刻state中抽取，但是后面 4xnf_al length相应的feature从上一时刻的state中获取，这部分信息的语义我暂时不太清楚
                    ...
                    # 这一步也是将最后的env相关的信息放入new_states中
                    ...
                    if self.args.inseparable:
                        ...
                        inputs_al = batch["state"][:, t, :self.nf_al * self.n_agents].reshape(batch.batch_size, self.n_agents, -1)
                        inputs_al = th.cat((inputs_al, th.eye(self.n_agents, device=batch.device).unsqueeze(0).expand(batch.batch_size, -1, -1)), -1)       # 将环境原本state中的ally units local state + one hot id
                        inputs_al = inputs_al.reshape(batch.batch_size * self.n_agents, -1)     # shape=(bs * n_agents, nf_al+n_agents)
                        inputs_en = batch["state"][:, t, self.nf_al * self.n_agents:self.nf_al * self.n_agents + self.n_enemies * self.nf_en].reshape(batch.batch_size * self.n_enemies, self.nf_en)
                        # inputs_en.shape=(bs * n_enemies, nf_en)
                        ...
                        # 最后这一步将环境中原有的env信息补充道new_states中
