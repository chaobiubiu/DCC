...
class LDSAMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), subtask_prob_logit.view(ep_batch.batch_size, self.n_agents, -1), subtask_embed
        # [bs, n_agents, n_actions], [bs, n_agents, n_subtasks], [bs, n_subtasks, embed_dim]

    def init_hidden(self, batch_size):
        self.hidden_states_subtask_policy = self.agent.init_hidden_subtask_policy().unsqueeze(0).expand(batch_size, self.n_agents, -1)  # bav
        self.hidden_states_agent_embed = self.agent.init_hidden_agent_embed().unsqueeze(0).expand(batch_size, self.n_agents, -1)  # bav
