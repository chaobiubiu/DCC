...
import numpy as np
import torch as th
from src.modules.bandits.const_lr import Constant_Lr
from src.modules.bandits.uniform import Uniform
from src.modules.bandits.reinforce_hierarchial import EZ_agent as enza
from src.modules.bandits.returns_bandit import ReturnsBandit as RBandit


class MavenParallelRunner:
    ...
    def __init__(self, args, logger):
        ...
        assert self.batch_size == 1, "maven is restricted to single thread."
        ...
        if 'mpe' in self.args.env or 'wild_rescue' in self.args.env:
            self.ps = [Process(target=env_worker, args=(worker_conn, CloudpickleWrapper(partial(env_fn, env_args=self.args.env_args, seed=self.args.env_args['seed']))))
    ...
    def setup(self, scheme, groups, preprocess, mac):
        ...
        if self.args.noise_bandit:
            if self.args.bandit_policy:
                print("Maven uses hierarchical policy.")
