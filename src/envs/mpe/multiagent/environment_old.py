...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, scenario=None, reset_callback=None, reward_callback=None,
        ...
        self.entity_permutation = None
        self.agent_recovery = None
        self.entity_recovery = None
    ...
    def step(self, action_n):
        ...
        # 2022-10-4, random decision ordering
        action_n = np.array(action_n)[self.agent_recovery].tolist()
        ...
        # advance world state
        self.world.step()
        # global states为 (max_num_agents+max_num_landmarks) X local_state_size 维度，
        ...
        # 2022-10-4, random decision ordering
        obs_n = np.array(obs_n)[self.agent_permutation]
    ...
    def permutate_idx(self):
        # self.agent_permutation = np.arange(self.max_num_agents)
        # curr_agent_permutation = np.random.permutation(self.scenario.num_agent)
        # self.agent_permutation[:self.scenario.num_agent] = curr_agent_permutation       # Only randomly permutate the current agents in current scenario.
        self.agent_permutation = np.random.permutation(self.max_num_agents)         # Only randomly permutate the agent ordering.
        landmark_order = np.arange(self.max_num_agents, (self.max_num_agents + self.max_num_landmarks))
        self.entity_order = np.concatenate([self.agent_permutation, landmark_order])
        self.agent_recovery = [np.where(self.agent_permutation == i)[0][0] for i in range(self.max_num_agents)]
        self.agent_recovery = np.array(self.agent_recovery)
        self.entity_recovery = [np.where(self.entity_order == i)[0][0] for i in range(self.max_num_agents + self.max_num_landmarks)]
        self.entity_recovery = np.array(self.entity_recovery)
    ...
    def get_env_info(self):
        if self.discrete_action_space:
            env_info = {"state_shape": self.scenario.state_size,
                        "obs_shape": self.scenario.state_size,
