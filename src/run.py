...
def run(_run, _config, _log):
    ...
    if 'dcc' in args.name:
        unique_token = "{}_discrim_{}_discrim_loss_w_{}_intrinsic_{}_per_weight_{}_global_weight_{}_n_skills_{}_skill_interval_{}_{}".format(args.name,
        args.use_discrim, args.discrim_loss_weight, args.use_intrinsic, args.low_per_weight, args.low_global_weight,
        args.n_skills, args.skill_interval, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M"))
    else:
        unique_token = "{}_{}".format(args.name, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))

    args.unique_token = unique_token
    if args.use_tensorboard:
        if args.env == 'sc2' or args.env == 'mpe':
            tb_logs_direc = os.path.join(dirname(dirname(abspath(__file__))), "results", "tb_logs", args.env, args.env_args['map_name'])
        else:
...
def run_sequential(args, logger):
    ...
    scheme = {
        ...
        "skills": {"vshape": (1,), "group": "agents", "dtype": th.long},      # High-level policy
    ...
    if args.checkpoint_path != "":
        ...
        if args.evaluate or args.save_replay:
            ...
            # import numpy as np
            # np.save("actions_batch.npy", runner.actions_batch)
            return
